...
- Module 2 (Injury & Recovery Logging) was **gutted by the HoloMotion-only cut (2026-08-02)** — the `Injury` and `SelfReport` models, `routes/injuries.js`, `routes/selfReports.js` and the self-report→injury promotion transaction are all **deleted**. What survives is a single clinician-set flag on the Athlete row (`isInjured` / `injuryNote` / `injuryBy` / `injuryAt`), written by `PATCH /api/athletes/:id/injury` (medical + admin). There is no injury table, no injury history and no athlete self-reporting. The 6-module FDD and Table 4.1 still describe the old module and need a matching rewrite before viva — that rewrite is JC's open item, so **do not renumber or rename the modules on your own**
