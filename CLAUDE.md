...
- **Sparse grains get their own chart type** (2026-08-11). Two periods draw a **slopegraph** — every metric as a line between them on one shared scale, so "ROM fell 5.2 while stability rose 2.6" is visible where two headcount columns showed only headcount. Line colour comes from the API's `direction`, never the sign of the delta, because exercise risks improve by going DOWN. One period shows its **composition** (the same rows one grain finer: year → quarters, quarter → months) rather than a number and an apology. See `docs/DESIGN_DECISIONS.md §26`
