...
| # | Question | Answer |
|---|---|---|
| 1 | UC-2 Reset Password — works today? | Yes, fully. `POST /api/auth/forgot-password` issues a single-use token (32 random bytes from `crypto.randomBytes`), stores its SHA-256 hash with a 60-minute TTL on the user row, and sends a branded HTML email via env-driven SMTP. `POST /api/auth/reset-password` consumes the token, validates against the password policy, and rotates via the existing bcrypt hook. The frontend is at `/forgot-password` and `/reset-password/[token]`. The flow is open to all three roles even though Slide 21 originally framed it as medical/admin-only — there's no defensible reason to lock athletes out of recovering their own accounts. |
...
| 4 | Why SHA-256 hash on the token, not bcrypt like the password? | Reset tokens are short-lived (60 min) and high-entropy (256-bit) — bcrypt's work-factor cost would only slow down the legitimate user. SHA-256 of a 256-bit random value is computationally infeasible to invert, and storing the hash means a DB compromise leaks no active tokens. Passwords get bcrypt because users pick low-entropy values and we want the work-factor against brute force. |
...
| 7 | Why only three roles — no coach / admin assistant? | Three is what ISN's operational model has: athletes self-log, medical staff clinically review, admin runs cohort analytics. Coaches at ISN currently feed via medical staff. Adding more roles is FYP II scope if ISN restructures. |
...
### Data Management (UC-20 to UC-23)
...
| 15 | Dataset removal cascade? | Re-upload performs upsert by `athleteId` (latest wins). Currently no full "remove imported batch" — the import log records imports, and removing a single import is the deferred UC-23 polish. No automatic cascade to derived data because Activity/Injury are independent of screening. |
