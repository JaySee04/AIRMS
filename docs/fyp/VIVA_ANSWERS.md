...
| 5a | Has the bulk-import axis changed since the comparison was drawn? | Strengthened — AIRMS now also ingests ISN's actual screening artefact, the image-only HoloMotion report PDF, via AI-assisted extraction with confirm-before-commit. None of the four compared systems can ingest an image-only clinical PDF; they all require structured exports, proprietary formats, or wearable telemetry. |
...
| 7 | Why only three roles — no coach / admin assistant? | Three is what ISN's operational model has: athletes self-log, medical staff clinically review, admin runs cohort analytics. Coaches at ISN currently feed via medical staff. Adding more roles is FYP II scope if ISN restructures. (If pressed: a read-only coach squad-readiness view exists in the codebase as an experimental spike de-risking that FYP II option — it is deliberately outside the FYP I artefact and reuses `classifyCompositeRisk()` unchanged, so it adds no new clinical logic.) |
...
| 14 | Why add a PDF ingestion path when Excel already worked? | Because Excel was never ISN's real artefact — Dr Thung's screening workflow produces a per-athlete HoloMotion report PDF. The Excel path forced someone to transcribe that report by hand. Ingesting the PDF directly removes the transcription step, which is exactly Dr Thung's "easy upload without complex steps" requirement. Excel remains as the secondary bulk path. |
...
| 2 | Where do the 15 / 25 thresholds come from? | From the instrument itself: the HoloMotion report's own legend bands 0–15 as Low Risk, and the medical dashboard already used >15 as its "elevated" convention — so Watch = the report's own Low/Medium boundary. 25 splits Watch from High inside the report's Medium band, giving conservative two-band alerting anchored to the source scale rather than invented numbers. |
| 3 | Isn't this "personalised thresholds" duplicating the composite model? | Different axis of personalisation: the composite model personalises *ACWR workload thresholds* by vulnerability; this layer personalises *which body regions matter* by sport. The practical effect is that a runner's knee alerts at >15 while their neck waits for >25 — region-level, workload-independent, and each is explainable on its own terms. |
