...
| [disciplines.ts](../frontend/src/lib/disciplines.ts) | `SPORT_DISCIPLINES` seed suggestions + `disciplinesForSport()` — the per-sport event autocomplete seeds (events are otherwise admin-extensible free strings) |
...
| [auth.ts](../frontend/src/lib/auth.ts) | `saveSession`, `getSession`, `clearSession`, `SessionUser` type, plus `hasPermission()` + `PermissionKey` for the medical-staff feature opt-out (mirrors backend `utils/permissions.js`) |
...
| [rank.ts](../frontend/src/lib/rank.ts) | **2026-08-12** `percentileFromRank()` (mid-rank `(r-0.5)/n`), `ordinal()`, `percentilePhrase()` — the cohort standing read as a **percentile** beside the raw rank, because "12th of 62" means nothing without the denominator in mind |
...
table below plus the four verification commands — `npm run mutate` (74 guards),
...
suites, typecheck, lint), **mutate** (the 74 guards — separate because it exceeds
...
Counts as of 2026-09-17: **60 backend suites / 858 tests**, **24 frontend suites / 415 tests**.
