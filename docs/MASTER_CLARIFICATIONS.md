...
- **`admin` is Dr Thung's role.** The admin surface — cohort norms and their governance, personnel, settings, the activity log, imports — was built at his request, and he is the person expected to sit in it: the stakeholder who has to address what is going on inside the system, not merely read reports about it. This is recorded because the role's PERMISSIONS were documented and its PERSONA was not, and the omission actively misleads: reasoning from "admin can change the norms" alone leads to withholding the account from the very person it was designed for. `executive` is the read-only alternative for someone who wants oversight WITHOUT that responsibility; it is not a safer substitute for the admin.

- **`executive` (5th role, added 2026-08-08 on JC's instruction).** Read-only institutional oversight for a senior non-technical stakeholder: the admin's Screening Analytics and Programme Activity pages, and the three PDF reports. No write access anywhere — no import, no norm editing, no roster or personnel changes, no settings, no Excel backup. Enforced by naming the role explicitly on each permitted route (`rbac('admin', 'executive')`); every other admin route still guards on `admin` alone, and `backend/tests/rbac.test.js` asserts the role cannot stand in for an admin. **Not a "super admin"** — it holds strictly fewer powers than `admin`, so that name would misdescribe it.
