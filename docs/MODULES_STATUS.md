...
| **Individual PDF — Lateral Symmetry section + posture in the interpretation** (TMG-inspired, 2026-07-29) — the per-region L/R subitems were only shown as raw numbers; now the report also has a **Lateral Symmetry** section (region · HoloMotion symmetry score on its own 85/75/60 tiers · plain-language status · which side is weaker, from L vs R ROM/stability), and the derived Interpretation now surfaces **posture deviations** (previously extracted but never interpreted) and graded symmetry findings. All values are printed on the HoloMotion report — no TMG-style contraction/tonus data is fabricated (different instrument) | ✅ | [`routes/screeningReports.js`](../backend/src/routes/screeningReports.js) (`symmetryFindings`, `symmetrySection`, `interpret`) |
