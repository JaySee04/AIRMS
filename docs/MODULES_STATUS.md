...
| **Coach view (FYP II — first-class 4th role)** — read-only squad readiness scoped to the coach's **one** assigned sport; athletes sorted worst-first with the worst region named, **filterable by programme / gender / event**; selecting a row opens a **read-only screening detail** (risk badge + radar + ScreeningPanel + body map + events), no clinical affordances; can download the team screening PDF for the sport (individual PDF stays blocked). **Coaching cockpit:** a *Needs-attention* list (Restricted + injured athletes, each with its persisted escalation reason), a *Squad focus* card (common weak spots + muscle hotspots + readiness-by-event + coverage + momentum), and a per-athlete **trend** arrow vs the previous screening | ✅ | [`/coach/dashboard`](../frontend/src/app/coach/dashboard/page.tsx), [`routes/coach.js`](../backend/src/routes/coach.js) |
