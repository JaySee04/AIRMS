...
| 6 | Clinical & Squad Monitoring | medical + coach | 🟢 **functional** — roster cohort verdict, clinician watchlist | `/medical/dashboard`, `/coach/dashboard` | `/api/athletes`, `/api/athletes/:id`, `/api/coach/readiness`, `/api/watchlist` |
...
Module 1 is the FYP showcase requiring no further iteration. Activity Tracking (the FYP I Module 1) was removed 2026-07-20 — see its historical section below. Module 2 was removed 2026-08-02 by the HoloMotion-only cut and is retained as a record, not a spec. Modules 3–6 are functional end-to-end across all four roles. **As of 2026-09-06 there are no deferred items in any module** — the last two, Module 5's PODIUM-vs-PELAPIS comparison and Module 6's clinician watchlist, both shipped that day (`DESIGN_DECISIONS.md` §68 and §66). One known gap remains and it is **external, not unbuilt**: Module 3's real ISN directory access, where the seam is mocked and ready pending Dr Thung granting it.
...
- **App-level hardening (2026-07)** — `helmet` security headers (HSTS, noSniff, frameguard; CORP relaxed to cross-origin only so the frontend can fetch streamed PDFs) and an `express-rate-limit` throttle on `/api/auth` (30 **failed** attempts / 15 min / IP, so a demo logging in/out across roles is never blocked while brute-force is). Database-backed since 2026-09-10; a successful sign-in **forgives the failures before it**, awaited inside the request — `skipSuccessfulRequests` was removed on 2026-09-11 because it un-counts from a post-response hook that a serverless host never runs, which had made the deployed limit "30 requests" rather than "30 failures" (`SILENT_FAILURES.md` 3r)
...
**What it does (full vision):** Medical staff's home page. Search/filter athletes, click into a per-athlete view that mirrors the athlete's own dashboard and adds the clinician's affordances — band override, injury-status flag, screening-date replay. This is essentially **the athlete dashboard, viewed by a clinician**.

> **Corrected 2026-09-09.** Five bullets below described the pre-2026-08-02
> system: an injury history list, a sport injury-pattern card, parallel injury
> fetching, and a "+ Log Injury" button deep-linking to `/medical/injury-log`
> keyed `ATH0001`. **None of those exist.** The HoloMotion-only cut deleted the
> `Injury` model and its page 38 days before this file last claimed them, and
> the athlete key became the IC number. What survives is the single clinician-set
> flag described below. The page's *actual* landing pane — a cohort-verdict
> shortlist and a watchlist — was listed as deferred while shipped.

**Current state (functional):**
- ✅ Page at `/medical/dashboard` loads the roster from `GET /api/athletes`
- ✅ Search bar (name + IC), Sport / Programme / Gender / Event filter dropdowns
- ✅ Athlete card grid with avatar + name + sport/programme + IC; click selects (active card has gold border)
- ✅ **The landing pane answers the role's own question** (2026-09-06, `DESIGN_DECISIONS.md §65`): roster stat tiles, a **"Who the cohort verdict flags"** shortlist worst-first off the server-resolved `latestBand`, and never-screened counted APART from any band. The instrument's own Exercise Risks ranking is kept beside it, labelled as the instrument's reading rather than the cohort verdict
- ✅ **The clinician's personal watchlist** (2026-09-06, `DESIGN_DECISIONS.md §66`), first among the quick groups; toggled from the athlete header. Medical + admin only, `settings`-backed, not audited
- ✅ On selection, fetches the full athlete record and screening history
...
- ✅ **Injury-status control** (`InjuryStatusControl`, `PATCH /api/athletes/:id/injury`) — the single clinician-set flag that survived the cut. Its purpose is cohort-norm eligibility, and setting it rebuilds the norm and rescores in the same request
...
| **On-screen screening history + athlete self-download (2026-07-23)** — `ScreeningHistory` table (newest-first rows + "Change since first" delta row, Ex. Risks coloured lower-is-better like the PDF) on the athlete dashboard and the medical + coach detail views; its API (`GET /screenings/athlete/:id`, previously caller-less) slimmed to summary columns and sport-scoped for coaches. The athlete dashboard's copy hosts a **Download PDF** button so athletes can pull their own individual report (UC-41 actor: "Athlete (own report only)"). **This did not work until 2026-08-20** — the note here claimed the actor was "already permitted server-side" and it was not: `athlete` was missing from the route's `rbac()` list, so the button returned "Access denied: insufficient role" on the athlete's own record from the day it shipped, and the self-scope check written for it two lines below was never once reached. Now admitted, scoped by two tested predicates in `utils/permissions.js`, and verified live (own report 200, another athlete 403, unknown id 403 rather than 404) | ✅ | [`ScreeningHistory.tsx`](../frontend/src/components/dashboard/ScreeningHistory.tsx), [`routes/screenings.js`](../backend/src/routes/screenings.js) |
