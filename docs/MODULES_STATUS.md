...
| **Athlete events / disciplines** — `athlete_disciplines` join table (an athlete can hold several events, e.g. badminton Men's Singles + Men's Doubles); badminton is the only curated sport so far (5 events), others blank; set on the PDF-import identity step, shown as chips on the medical + coach athlete views, filterable everywhere. Per-sport catalogue is frontend-only | ✅ | [`models/AthleteDiscipline.js`](../backend/src/models/AthleteDiscipline.js), [`lib/disciplines.ts`](../frontend/src/lib/disciplines.ts), [`routes/athletes.js`](../backend/src/routes/athletes.js) |
