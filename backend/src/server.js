...
const { SettingsRateLimitStore, authThrottleKey } = require('./utils/rateLimitStore');
...
// FAILURES BEFORE IT. That second clause is the mechanism, and it replaced
// `skipSuccessfulRequests` on 2026-09-11 for a measured reason.
//
// What the policy is for: a demo runs off one laptop (one IP) and signs in and
// out across five roles many times, all SUCCESSFULLY; those must never be
...
//
// WHY NOT `skipSuccessfulRequests`. It decrements from a `res.on('finish')`
// handler — after the response is flushed. On a long-lived process that write
// completes; on Vercel the instance freezes once the response is out, so the
// decrement is issued and never lands. Measured against the hosted API:
// five consecutive SUCCESSFUL logins took `remaining` 28 → 27 → 26 → 25 → 24,
// never recovering. The deployed limiter was therefore "30 REQUESTS / 15 min",
// not "30 failures" — while this comment, the docs and the RateLimit header all
// said otherwise. A clinic behind one NAT address would have locked itself out,
// and nothing would have reported why.
//
// So the forgiveness happens INSIDE the request instead: `routes/auth.js`
// awaits `clearRateLimit()` on a successful sign-in, before responding. It
// depends on nothing the platform may decline to run after the fact. The key is
// generated by an explicitly-named function shared with that call site, because
// a reset computed against a different key than the limiter counts on would
// silently forgive nothing.
...
const authLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  limit: 30,
  // Named and shared rather than left to the library default, so the reset in
  // routes/auth.js provably clears the counter this limiter reads.
